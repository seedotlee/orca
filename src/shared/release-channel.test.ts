...
describe('release channel', () => {
  ...
  it('keeps dev builds out of the main release repo, and apart from each other', () => {
    expect(getReleaseRepoForChannel('hourly')).toBe('seedotlee/orca-hourly')
    expect(getReleaseRepoForChannel('daily')).toBe('seedotlee/orca-daily')
    ...
    expect(getReleaseRepoForChannel('adhoc')).toBe('seedotlee/orca-adhoc')
    expect(getReleaseRepoForChannel('stable')).toBe('seedotlee/orca')
    expect(getReleaseRepoForChannel('rc')).toBe('seedotlee/orca')
  })
  ...
  it('builds release-notes links against the repo that published the version', () => {
    ...
    expect(getReleaseNotesUrlForVersion('1.4.160-daily.202607281300')).toBe(
      'https://github.com/seedotlee/orca-daily/releases/tag/v1.4.160-daily.202607281300'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160')).toBe(
      'https://github.com/seedotlee/orca/releases/tag/v1.4.160'
    )
    expect(getReleaseNotesUrlForVersion('v1.4.160-rc.3')).toBe(
      'https://github.com/seedotlee/orca/releases/tag/v1.4.160-rc.3'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160-adhoc.20260728140533')).toBe(
      'https://github.com/seedotlee/orca-adhoc/releases/tag/v1.4.160-adhoc.20260728140533'
    )
    expect(getReleaseNotesUrlForVersion(null)).toBe('https://github.com/seedotlee/orca/releases')
  })
  ...
  it('sorts consecutive hourly builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/seedotlee/orca-hourly/releases/tag/v${version}`,
      installerUrl: null
    })
  ...
  it('sorts consecutive adhoc builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/seedotlee/orca-adhoc/releases/tag/v${version}`,
      installerUrl: null
    })
    const sorted = sortReleaseBuildsNewestFirst([
