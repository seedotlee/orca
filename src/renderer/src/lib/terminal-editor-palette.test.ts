...
describe('resolveTerminalEditorPalette', () => {
  ...
  it('maps syntax roles onto the ANSI palette and mutes comments toward the background', () => {
    ...
    // Why: Solarized Light's foreground is only ~5:1 on its background, so the contrast gate
    // cannot mute comments below the foreground itself; they stay legible instead of fading.
    expect(palette?.syntax.comment).toBe('#657b83')
  })
