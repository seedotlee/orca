...
describe('parseTccPromptEvent', () => {
  ...
  it('separates the accessing binary from the responsible app', () => {
    ...
    expect(event?.responsibleIdentifier).toBe('com.seedotlee.orca')
...
describe('isOrcaAttributedPrompt', () => {
  it('accepts the app and detached terminal helper across Orca build identities', () => {
    for (const id of [
      'com.seedotlee.orca',
      'com.seedotlee.orca.helper',
      'com.seedotlee.orca.dev',
      'com.seedotlee.orca.dev.helper',
      'com.seedotlee.orca.local',
      'com.seedotlee.orca.local.helper'
    ]) {
      expect(
        isOrcaAttributedPrompt({
  ...
  it('rejects unrelated services even when Orca is responsible', () => {
    expect(
      isOrcaAttributedPrompt({
        ...
        responsibleIdentifier: 'com.seedotlee.orca'
      })
    ).toBe(false)
  })
})
